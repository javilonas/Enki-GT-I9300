/*
 * Author Rem Sound: javilonas, 01.11.2014
 *
 * Version 1.2
 *
...
// Do not touch dac direct at all when P4NOTE
	...
	if (is_headphone != current_is_headphone)
		...
		if (debug(DEBUG_NORMAL))
			printk("Rem-sound: Output new status - %s\n", is_headphone ? "Headphone" : "Speaker");
	...
	if (is_call != current_is_call)
	{
		...
		if (debug(DEBUG_NORMAL))
			printk("Rem-sound: Call detection new status - %s\n", is_call ? "in call" : "not in call");
	...
	if (is_fmradio != current_is_fmradio)
        {
                ...
                if (debug(DEBUG_NORMAL))
                        printk("Rem-sound: FM radio detection new status - %s\n", is_fmradio ? "active" : "inactive");
...
{
	if (debug(DEBUG_NORMAL))
		printk("Rem-sound: start applying settings after %d micro seconds delay\n", change_delay);
	...
	if (debug(DEBUG_NORMAL))
		printk("Rem-sound: done applying settings after %d micro seconds delay\n", change_delay);
...
{
	// print current value
	return sprintf(buf, "Rem change delay: %d\n", change_delay);
}
					...
					const char *buf, size_t count)
{
	...
	if ((val >= MIN_CHANGE_DELAY) && (val <= MAX_CHANGE_DELAY))
	{
		// print debug info
		if (debug(DEBUG_NORMAL))
			printk("Rem-sound: change delay %d micro seconds\n", change_delay);
